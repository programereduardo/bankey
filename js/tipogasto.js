...
$('#registrar').on('click', function(){
  ...
  for (var i = 0; i < acciones.length; i++) {
    if (acciones[i]['mod_nombre'] == 'Tipo gastos') {
...
var guardarDocumento = function(){
  ...
  if (error === true) {
    ...
    $.ajax({
      url: 'tipogasto_controller/guardar_tipodocumentos',
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false
...
var obtener_documentos = function(){
  ...
  var modelFila = '<tr>'+
      ...
      $.ajax({
        url: 'tipogasto_controller/obtener_tipodocumentos',
...
function mostrarAyudaClave() {
  $.notify({
    message: 'Ingrese un máximo de 6 (seis) caracteres. Esto le ayudara a diferenciar un tipo de gasto de otro.'
  }, {
    type: 'info',
    delay: 3000,
    placement: {
  ...
  if (invalidDelete === true) {
    ...
    bootbox.confirm({
      ...
      callback: function(result){
        if (result === true) {
          $.ajax({
            url: 'tipogasto_controller/inactivar_tipodocumentos',
            type: 'POST',
            data:{
