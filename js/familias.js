...
$('#registrar').on('click', function(){
  ...
  for (var i = 0; i < acciones.length; i++) {
    if (acciones[i]['mod_nombre'] == 'Familias') {
...
var guardarFamilia = function(){
  var btnSavingFamilia = $(this);
  var datos_familia = $('[name=formSaveFamily]').serializeArray();
  var error = false;
  var mensajeError = 'Guardado correctamente.';
  for (var i = 0; i < datos_familia.length; i++) {
    var label = datos_familia[i]['name'];
    var valor = datos_familia[i]['value'];
  ...
  if (error === true) {
    ...
    $('[name=modalFamilia]').modal('hide');
    waitingDialog.show('Guardando los datos, por favor espere...',
    {dialogSize: 'm', progressType:''});
    btnSavingFamilia.attr('disabled', 'disabled');
    $.ajax({
      url: 'familia_controller/guardar_familia',
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false
      ...
      waitingDialog.hide();
      btnSavingFamilia.removeAttr('disabled');
      $.notify({
        message: mensajeError
      }, {
        type: 'success',
...
$('[name=btnSavingFamilia]').on('click', guardarFamilia);
//fin llamar funcion guardar familia


//Inicio obtener familias
var componenteListarFamilias = $('[name=listar_familias]');
var obtener_familias = function(){
  waitingDialog.show('Cargando, por favor espere...', {dialogSize: 'm', progressType: ''});
  var modelFila = '<tr>'+
      '         <th scope="row">'+
      '            <span name="btnEditar" id="editar_familia"'+
      '              codigo_familia="{0}" clave_familia="{1}" abreviatura_familia="{2}" detalle_familia="{3}"'+
      ...
      '            <span name="btnEliminar" codigo_familia="{0}" title="Eliminar" '+
      ...
      $.ajax({
        url: 'familia_controller/obtener_familias',
        success: function(response){
          var respuesta = $.parseJSON(response);
          componenteListarFamilias.empty();
          if (respuesta.success === true) {
            var datos = respuesta.familias;
            console.log(datos)
            for (var i = 0; i < datos.length; i++) {
              componenteListarFamilias.append(modelFila.format(
                datos[i]['tipcodigo'], //0
                datos[i]['tipclave'], //1
            ...
            $('[name=btnEliminar]').on('click', eliminar_familia);
            $('[name=btnEditar]').on('click', modificar_familia);
          }
          waitingDialog.hide();
        }
      })
...
function mostrarAyudaClave() {
  $.notify({
    message: 'Ingrese tres caracteres. Esto le ayudara a diferenciar una familia de otra.'
  }, {
    type: 'info',
    delay: 3000,
    placement: {
...
//Inicio funcion eliminar_familia

var eliminar_familia = function(){
  ...
  if (invalidDelete === true) {
    ...
    var codigo_familia = $(this).attr("codigo_familia");
    bootbox.confirm({
      title: 'Confirmación',
      ...
      callback: function(result){
        if (result === true) {
          $.ajax({
            url: 'familia_controller/inactivar_familia',
            type: 'POST',
            data:{
...
var modificar_familia = function() {
  ...
  if (invalidChange === true) {
    ...
    var codigo_familia = $(this).attr("codigo_familia");
    var clave_familia = $(this).attr("clave_familia");
    var abreviatura_familia = $(this).attr("abreviatura_familia");
    var detalle_familia = $(this).attr("detalle_familia");
    $('[name=modalFamilia]').modal();
    $('[name=modalFamilia]').find('.modal-title').text('Modificar familias');
    $('[name=modalFamilia]').find('[name=codigo_familia]').val(codigo_familia);
    $('[name=modalFamilia]').find('[name=clave]').val(clave_familia);
    $('[name=modalFamilia]').find('[name=abreviatura]').val(abreviatura_familia);
    $('[name=modalFamilia]').find('[name=detalle]').val(detalle_familia);
    $('[name=modalFamilia]').find('[name=tipo]').val('2');
  }
}
...
var limpiar_familia = function(){
  $('[name=modalFamilia]').find('[name=codigo_familia]').val("");
  $('[name=modalFamilia]').find('[name=clave]').val("");
  $('[name=modalFamilia]').find('[name=abreviatura]').val("");
  $('[name=modalFamilia]').find('[name=detalle]').val("");
  $('[name=modalFamilia]').find('[name=tipo]').val('1');
}
