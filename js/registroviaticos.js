...
$('#registrar').on('click', function(){
  ...
  for (var i = 0; i < acciones.length; i++) {
    if (acciones[i]['mod_nombre'] == 'Registro viaticos') {
...
var guardarDocumento = function(){
  ...
  if (error === true) {
    ...
    $.ajax({
      url: 'registroviaticos_controller/guardar_tipodocumentos',
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false
...
var obtener_documentos = function(){
  ...
  var modelFila = '<tr>'+
      ...
      $.ajax({
        url: 'registroviaticos_controller/obtener_tipodocumentos',
  ...
  if (invalidDelete === true) {
    ...
    bootbox.confirm({
      ...
      callback: function(result){
        if (result === true) {
          $.ajax({
            url: 'registroviaticos_controller/inactivar_tipodocumentos',
            type: 'POST',
            data:{
