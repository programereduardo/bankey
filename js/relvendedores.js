...
var guardarDocumento = function(){
  ...
  for (var i = 0; i < datos_documento.length; i++) {
    ...
    switch (label) {
      case 'vendedor1':
        if (valor.trim() == ''){
          mensajeError = 'Ingrese el vendedor número uno (1).';
          error = true;
          compItem.focus();
      ...
      case 'vendedorid1':
        if (valor.trim() == ''){
          mensajeError = 'Error! El vendedor número uno (1) no se encuentra registrado.';
          error = true;
          compItem.focus();
          compItem.parent('div').addClass("has-error");
          i = datos_documento.length + 100;
      ...
      case 'vendedor2':
      if (valor.trim() == ''){
        mensajeError = 'Ingrese el vendedor número dos (2).';
        error = true;
        compItem.focus();
        compItem.parent('div').addClass("has-error");
        i = datos_documento.length + 100;
      ...
      case 'vendedorid2':
        if (valor.trim() == ''){
          mensajeError = 'Error! El vendedor número dos (2) no se encuentra registrado.';
          error = true;
          compItem.focus();
          compItem.parent('div').addClass("has-error");
          i = datos_documento.length + 100;
  ...
  if (error === true) {
    ...
    $.ajax({
      url: 'relvendedores_controller/guardar_tipodocumentos',
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false,
...
var obtener_documentos = function(){
  ...
  var modelFila = '<tr>'+
      ...
      $.ajax({
        url: 'relvendedores_controller/obtener_tipodocumentos',
          ...
          if (respuesta.success === true) {
            ...
            for (var i = 0; i < datos.length; i++) {
              componenteListarDocumentos.append(modelFila.format(
                ...
                datos[i]['ternombre'], //1
                datos[i]['vendedor'], //2
                datos[i]['relvenestado'], //3
                datos[i]['relvenobservacion'], //4
                datos[i]['relvenprincipal'],//5
                datos[i]['relvensecundario'],//6
              ));
            }
